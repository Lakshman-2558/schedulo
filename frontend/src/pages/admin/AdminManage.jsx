...
const AdminManage = () => {
  ...
  const handleFileSelect = async (type, file) => {
    ...
    try {
      ...
      let endpoint = '';
      if (type === 'exam') {
        endpoint = '/upload/exam-timetable/preview';
        formData.append('examType', examType);
      }
      else if (type === 'classroom') endpoint = '/upload/classrooms/preview';
      else endpoint = '/upload/faculty/preview';
  ...
  const handleSave = async (type) => {
    ...
    try {
      ...
      let endpoint = '';
      if (type === 'exam') {
        endpoint = '/upload/exam-timetable';
        formData.append('examType', examType);
      }
      else if (type === 'classroom') endpoint = '/upload/classrooms';
      else endpoint = '/upload/faculty';
      const response = await api.post(endpoint, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
  ...
  const uploadSections = [
    ...
    },
    {
      ...
      description: 'Upload CSV or Excel file containing faculty information for allocation',
      accept: '.csv,.xlsx,.xls',
    },
  ]

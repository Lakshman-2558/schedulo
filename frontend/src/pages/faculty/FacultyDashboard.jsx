...
import { Calendar, Clock, AlertCircle, Download, Bell, MapPin, BookOpen } from 'lucide-react'
                ...
                >
                  ...
                  <div className="flex space-x-2 flex-shrink-0">
                    <button
                      onClick={async () => {
                        try {
                          await api.post(`/api/faculty/acknowledge/${duty._id}`, { action: 'acknowledge' })
          ...
          <div className="space-y-4">
            {dashboard?.upcomingDuties && dashboard.upcomingDuties.length > 0 ? (
              dashboard.upcomingDuties.map((duty) => (
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      {/* Exam Name */}
                      <h3 className="font-semibold text-gray-900 text-lg mb-1">{duty.exam?.examName}</h3>
                      ...
                      <div className="flex flex-wrap items-center gap-3 mb-2">
                        ...
                        <span className="flex items-center gap-1.5 text-sm text-gray-700">
                          ...
                          <span className="font-medium">{duty.startTime} - {duty.endTime}</span>
                        </span>
                      </div>

                      {/* Classroom and Campus Tags */}
                      <div className="flex flex-wrap items-center gap-2 mt-3">
                        {duty.classroom && (
                          <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                            <MapPin className="w-3 h-3" />
                            Room {duty.classroom.roomNumber}
                            {duty.classroom.block && ` - ${duty.classroom.block}`}
              ...
              <div className="text-center py-12">
                ...
                <p className="text-gray-500">No upcoming duties</p>
              </div>
            )}
          </div>
        </div>

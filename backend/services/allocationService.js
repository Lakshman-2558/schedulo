...
class AllocationService {
  /**
   * Convert FacultyUpload ID to FacultyCredentials ID by matching email
   * Falls back to FacultyUpload ID if no credentials found (for backward compatibility)
   */
  async getFacultyCredentialsId(facultyUploadId) {
    try {
      const facultyUpload = await FacultyUpload.findById(facultyUploadId);
      if (!facultyUpload) {
        console.warn(`⚠️  FacultyUpload not found: ${facultyUploadId}`);
        return facultyUploadId; // Return original ID as fallback
      }
      ...
      if (!facultyCredentials) {
        console.warn(`⚠️  No login credentials found for ${facultyUpload.email} (${facultyUpload.name}). Using FacultyUpload ID.`);
        return facultyUploadId; // Return FacultyUpload ID as fallback
      }

      console.log(`✅ Matched ${facultyUpload.name} (${facultyUpload.email}) to FacultyCredentials`);
      return facultyCredentials._id;
    } catch (error) {
      console.error('Error converting FacultyUpload to FacultyCredentials:', error);
      return facultyUploadId; // Return original ID as fallback
    }
  }
  ...
  async previewAllocation(examIds = null, campus = null, department = null) {
    try {
      ...
      let faculty = await FacultyUpload.find(excelFacultyQuery);
      if (faculty.length === 0) {
        faculty = await FacultyUpload.find(baseFacultyQuery);
      }
  ...
  async allocateForSession(session, allRooms, faculty, workloadMap, campus = null, examType = 'semester') {
    ...
    for (const room of allRooms) {
      ...
      for (const facultyMember of selectedFaculty) {
        // Convert FacultyUpload ID to FacultyCredentials ID
        const facultyCredentialsId = await this.getFacultyCredentialsId(facultyMember._id);

        const allocation = await Allocation.create({
          exam: referenceExam._id, // Link to first exam for reference
          classroom: room._id, // Ensure classroom is set - CRITICAL!
          faculty: facultyCredentialsId,  // Use FacultyCredentials ID for login matching
          date: date,
          startTime: examStart,
          endTime: examEnd,

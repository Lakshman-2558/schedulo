...
const { Admin, Hod, FacultyCredentials, getModelByRole } = require('../utils/userHelper');
...
class FileUploadService {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        try {
          ...
          rowData.parsedData = {
            ...
            examType: examType, // Use exam type from UI selection
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        try {
          ...
          // Try flexible parsing (last resort)
            ...
            const year = examDate.getFullYear();
            if (year < 2000 || year > 2100) {
              errors.push(`Row ${i + 2}: Invalid date format (year out of range: ${year}). Please use YYYY-MM-DD or DD-MM-YYYY format.`);
              continue;
            }
          }

          if (!examDate || isNaN(examDate.getTime())) {
            errors.push(`Row ${i + 2}: Invalid date format "${dateStr}". Supported formats: YYYY-MM-DD, DD-MM-YYYY, DD/MM/YYYY, MM/DD/YYYY`);
            continue;
          }
    ...
    try {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        try {
          ...
          const existingFaculty = await FacultyCredentials.findOne({ email });
          ...
          if (row.employeeId) {
            const existingByEmployeeId = await FacultyCredentials.findOne({ employeeId: row.employeeId.toString().trim() });
    ...
    try {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        try {
          ...
          // Faculty uploads always go to FacultyUpload collection (for allocation)
          // Not FacultyCredentials (for login)
          const UserModel = FacultyUpload;
      ...
      // Step 3: Prepare updates and new users

      for (const validRow of validRows) {
        const { row, email, UserModel, subjectsArray, rowIndex } = validRow;
        ...
        if (existing) {
          ...
          // Prepare new faculty upload entry (no password needed)
          const userData = {
            name: row.name.toString().trim(),
            email: email,
      ...
      // No password hashing needed for FacultyUpload (allocation only, not authentication)

      // Step 4: Batch update existing users using bulkWrite (faster)
      if (updates.length > 0) {
  ...
  /**
   * Preview faculty credentials (parse and validate without saving)
   */
  async previewFacultyCredentials(filePath, fileType) {
    try {
      const data = await this.parseFile(filePath, fileType);
      const previewData = [];
      const errors = [];
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        try {
          // Validate required fields
          const requiredFields = ['name', 'email', 'employeeId', 'department', 'campus'];
          const missingFields = requiredFields.filter(field => {
            const value = row[field];
            return !value || (typeof value === 'string' && value.trim() === '');
          });
          ...
          const existingEmail = await FacultyCredentials.findOne({ email });
          if (existingEmail) {
            rowData.warnings.push(`Email ${email} already exists in database`);
            warnings.push(`Row ${i + 2}: Email already exists`);
          }

          // Check if employeeId already exists
          const employeeId = row.employeeId.toString().trim();
          const existingEmployeeId = await FacultyCredentials.findOne({ employeeId });
          if (existingEmployeeId) {
            rowData.warnings.push(`Employee ID ${employeeId} already exists in database`);
            warnings.push(`Row ${i + 2}: Employee ID already exists`);
          }
          ...
          rowData.parsedData = {
            ...
            campus: row.campus.toString().trim(),
            phone: row.phone ? row.phone.toString().trim() : undefined,
            passwordGenerated: true // Indicate password will be generated
          };
      ...
      throw new Error(`Error previewing faculty credentials: ${error.message}`);
    }
  }

  /**
   * Upload faculty credentials (creates login accounts with generated passwords)
   */
  async uploadFacultyCredentials(filePath, fileType) {
    try {
      const data = await this.parseFile(filePath, fileType);
      const FacultyCredentials = require('../models/FacultyCredentials');
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        try {
          ...
          // Check for duplicate employeeIds in the same file
          if (employeeIdMap.has(employeeId)) {
            errors.push(`Row ${i + 2}: Duplicate employee ID ${employeeId} found in upload file`);
            continue;
          }
          employeeIdMap.set(employeeId, i);
      ...
      const existingByEmail = await FacultyCredentials.find({
        email: { $in: emails }
      }).select('name email employeeId department campus phone isActive').lean();

      const existingByEmployeeId = await FacultyCredentials.find({
        employeeId: { $in: employeeIds }
      }).select('name email employeeId department campus phone isActive').lean();

      const existingEmailsMap = new Map(existingByEmail.map(u => [u.email, u]));
      const existingEmployeeIdsMap = new Map(existingByEmployeeId.map(u => [u.employeeId, u]));
      const duplicates = [];

      // Step 3: Create new faculty credentials
      for (const validRow of validRows) {
        const { row, email, employeeId, rowIndex } = validRow;

        // Skip if already exists
        if (existingEmailsMap.has(email)) {
          const existingFaculty = existingEmailsMap.get(email);
          errors.push(`Row ${rowIndex}: Email ${email} already exists in database`);
          duplicates.push({
            rowIndex,
            reason: 'email',
            uploadData: {
            ...
            existingData: existingFaculty
          });
          continue;
        }

        if (existingEmployeeIdsMap.has(employeeId)) {
          const existingFaculty = existingEmployeeIdsMap.get(employeeId);
          errors.push(`Row ${rowIndex}: Employee ID ${employeeId} already exists in database`);
          duplicates.push({
            rowIndex,
            reason: 'employeeId',
            uploadData: {
        ...
        // Create faculty credentials (password will be auto-hashed by model)
        const facultyData = {
          name: row.name.toString().trim(),
          email: email,
        ...
        try {
          const faculty = await FacultyCredentials.create(facultyData);
          credentialsCreated.push(faculty._id);
          ...
          try {
            ...
            console.error(`Failed to send email to ${faculty.email}:`, emailError);
          }
        } catch (createError) {
          errors.push(`Row ${rowIndex}: ${createError.message}`);
        }
      }

      return {
        success: errors.length === 0,
        credentialsCreated: credentialsCreated.length,
        emailResults: emailResults,
        duplicates: duplicates.length > 0 ? duplicates : undefined,
        errors: errors.length > 0 ? errors : undefined
      };
    } catch (error) {
      throw new Error(`Error uploading faculty credentials: ${error.message}`);
    }
  }
}

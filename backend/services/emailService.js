...
class EmailService {
  ...
  /**
   * Send faculty credentials email (for new account creation)
   */
  async sendFacultyCredentials(faculty, generatedPassword) {
    const universityName = process.env.UNIVERSITY_NAME || 'Vignan University';
    const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:5173';
    const supportEmail = process.env.EXAM_CELL_EMAIL || 'examcell@vignan.edu';

    const subject = `Welcome to ${universityName} - Your Login Credentials`;

    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #f9fafb;">
        
        <!-- Header -->
        <div style="background: linear-gradient(135deg, #1a56db 0%, #6366f1 100%); padding: 30px; border-radius: 12px 12px 0 0; text-align: center;">
        ...
        <div style="background: #ffffff; padding: 30px; border-radius: 0 0 12px 12px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
          ...
          <p style="color: #374151; font-size: 16px; margin: 0 0 20px 0;">
            Dear <strong>${faculty.name}</strong>,
          </p>
          
          <p style="color: #374151; font-size: 14px; line-height: 1.6; margin: 0 0 20px 0;">
            Your faculty account has been successfully created in our Exam Invigilation Management System. 
            You can now access the system to view your invigilation duty assignments, calendar, and analytics.
          </p>
          
          <!-- Credentials Box -->
          <div style="background: #f3f4f6; border-left: 4px solid #1a56db; padding: 20px; border-radius: 8px; margin: 25px 0;">
            ...
            <table style="width: 100%; border-collapse: collapse;">
              <tr>
                ...
                <td style="padding: 8px 0; color: #1f2937; font-size: 14px; font-family: 'Courier New', monospace; background: #ffffff; padding: 8px 12px; border-radius: 4px;">
                  ${faculty.employeeId}
                </td>
              </tr>
              <tr>
          ...
          <div style="text-align: center; margin: 30px 0;">
            <a href="${frontendUrl}/login" 
               style="display: inline-block; background: linear-gradient(135deg, #1a56db 0%, #6366f1 100%); color: #ffffff; padding: 14px 32px; text-decoration: none; border-radius: 8px; font-weight: 600; font-size: 16px; box-shadow: 0 4px 6px rgba(26, 86, 219, 0.3);">
          ...
          <!-- Instructions -->
          <div style="margin: 25px 0;">
            <h3 style="color: #1f2937; margin: 0 0 12px 0; font-size: 16px;">Getting Started</h3>
            <ol style="color: #374151; font-size: 14px; line-height: 1.8; margin: 0; padding-left: 20px;">
              <li>Click the "Login to Your Account" button above or visit: <a href="${frontendUrl}/login" style="color: #1a56db;">${frontendUrl}/login</a></li>
              <li>Enter your Employee ID and the password provided above</li>
              <li>Change your password from the profile settings</li>
              <li>Explore your dashboard to view duty assignments and calendar</li>
            </ol>
          </div>
          ...
          <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #e5e7eb;">
            <p style="color: #6b7280; font-size: 13px; margin: 0; line-height: 1.6;">
              If you have any questions or need assistance, please contact the Exam Cell at 
   ...
   */
  async sendPasswordReset(faculty, newPassword) {
    const universityName = process.env.UNIVERSITY_NAME || 'Vignan University';
    const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:5173';
    const supportEmail = process.env.EXAM_CELL_EMAIL || 'examcell@vignan.edu';

    const subject = `Password Reset - ${universityName}`;
    ...
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #f9fafb;">
        ...
        <div style="background: #ffffff; padding: 30px; border-radius: 0 0 12px 12px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
          ...
          <!-- Credentials Box -->
          <div style="background: #f3f4f6; border-left: 4px solid #1a56db; padding: 20px; border-radius: 8px; margin: 25px 0;">
            <h3 style="color: #1f2937; margin: 0 0 15px 0; font-size: 16px;">Your New Login Credentials</h3>
          ...
          <!-- Security Notice -->
          <div style="background: #fee2e2; border-left: 4px solid #dc2626; padding: 15px; border-radius: 8px; margin: 25px 0;">
            <p style="color: #991b1b; font-size: 14px; margin: 0; line-height: 1.6;">
              <strong>🔒 Security Notice:</strong> If you did not request this password reset, please contact the Exam Cell immediately at 
   ...
   */
  async sendPasswordResetOTP(faculty, otp) {
    const universityName = process.env.UNIVERSITY_NAME || 'Vignan University';
    const supportEmail = process.env.EXAM_CELL_EMAIL || 'examcell@vignan.edu';

    const subject = `Password Reset OTP - ${universityName}`;
    ...
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #f9fafb;">
        ...
        <div style="background: #ffffff; padding: 30px; border-radius: 0 0 12px 12px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
          ...
          <!-- OTP Box -->
          <div style="background: linear-gradient(135deg, #f0f9ff 0%, #e0f2fe 100%); border: 2px solid #1a56db; padding: 30px; border-radius: 12px; margin: 25px 0; text-align: center;">
            <p style="color: #1e293b; font-size: 14px; margin: 0 0 10px 0; font-weight: 600; text-transform: uppercase; letter-spacing: 1px;">
              Your OTP Code
            </p>
            <div style="background: #ffffff; padding: 20px; border-radius: 8px; display: inline-block; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
              <p style="color: #1a56db; font-size: 36px; font-weight: 800; margin: 0; font-family: 'Courier New', monospace; letter-spacing: 8px;">
                ${otp}
              </p>
            </div>
          ...
          <div style="background: #fef3c7; border-left: 4px solid #f59e0b; padding: 15px; border-radius: 8px; margin: 25px 0;">
            <p style="color: #92400e; font-size: 14px; margin: 0; line-height: 1.6;">
              <strong>⚠️ Important:</strong> Do not share this OTP with anyone. Our staff will never ask for your OTP.
            </p>
          </div>
          
          <!-- Instructions -->
          <div style="margin: 25px 0;">
            <h3 style="color: #1f2937; margin: 0 0 12px 0; font-size: 16px;">Next Steps</h3>
            <ol style="color: #374151; font-size: 14px; line-height: 1.8; margin: 0; padding-left: 20px;">
              <li>Enter the OTP code shown above in the password reset form</li>
          ...
          <div style="background: #fee2e2; border-left: 4px solid #dc2626; padding: 15px; border-radius: 8px; margin: 25px 0;">
            <p style="color: #991b1b; font-size: 14px; margin: 0; line-height: 1.6;">
              <strong>🔒 Security Notice:</strong> If you did not request this password reset, please ignore this email and contact the Exam Cell immediately at 
              <a href="mailto:${supportEmail}" style="color: #dc2626; text-decoration: none;">${supportEmail}</a>
            </p>
          </div>

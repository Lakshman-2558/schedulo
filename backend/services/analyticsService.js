...
const { FacultyCredentials, Admin, Hod } = require('../utils/userHelper');
...
class AnalyticsService {
  ...
  async getAdminAnalytics(filters = {}) {
    try {
      ...
      const [exams, allocations, totalFaculty, totalConflicts] = await Promise.all([
        ...
        FacultyCredentials.countDocuments(facultyQuery),
  ...
  async getFacultyWorkloadAnalytics(allocations, facultyQuery) {
    ...
    if (facultyIds.length > 0) {
      const facultyMembers = await FacultyCredentials.find({
        _id: { $in: facultyIds },
        ...facultyQuery
  ...
  async getFacultyAnalytics(facultyId, filters = {}) {
    try {
      ...
      const faculty = await FacultyCredentials.findById(facultyId).lean();
  ...
  async getDepartmentAverage(department, baseQuery) {
    ...
    const facultyInDept = await FacultyCredentials.countDocuments({
      department,
      isActive: true
    });

...
async function findUserById(userId) {
  ...
  try {
    ...
    user = await FacultyCredentials.findById(userId).select('-password').lean();
    if (user) {
...
async function findUserByEmail(email) {
  ...
  user = await FacultyCredentials.findOne({ email: normalizedEmail });
  if (user) return { user, model: FacultyCredentials, role: 'faculty' };
...
async function findUserByEmployeeId(employeeId) {
  ...
  user = await FacultyCredentials.findOne({ employeeId: normalizedId });
  if (user) return { user, model: FacultyCredentials, role: 'faculty' };
...
async function findUserForLogin(employeeId) {
  ...
  user = await FacultyCredentials.findOne({ employeeId: normalized });
  if (user) {
    console.log(`✅ Found Faculty by employeeId: ${normalized}`);
    return { user, model: FacultyCredentials, role: 'faculty' };
  }
...
async function createUser(userData) {
  ...
  switch (role) {
    ...
    case 'faculty':
    default:
      console.log('   📝 Using FacultyCredentials model (collection: facultycredentials)');
      createdUser = await FacultyCredentials.create({ ...data, role: 'faculty' });
      console.log(`   ✅ Created faculty in 'facultycredentials' collection: ${createdUser._id}`);
      return createdUser;
  }
}

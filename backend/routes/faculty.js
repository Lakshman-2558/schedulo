...
const { FacultyCredentials, findUserById } = require('../utils/userHelper');
...
router.get('/dashboard', async (req, res) => {
  try {
    ...
    // Get allocations with populated exam and classroom
    const allocations = await Allocation.find({
      faculty: facultyId,
      status: { $ne: 'cancelled' }
    })
      .populate('exam', 'examName courseCode courseName date startTime endTime examType')
      .populate('classroom', 'roomNumber block floor building campus capacity')

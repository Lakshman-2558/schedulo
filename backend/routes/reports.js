...
router.get('/pdf', async (req, res) => {
  try {
    ...
    if (facultyIds.length > 0) {
      const facultyMembers = await FacultyCredentials.find({ _id: { $in: facultyIds } })
...
router.get('/duty-letter/:allocationId', async (req, res) => {
  try {
    ...
    let faculty = null;
    if (facultyId) {
      faculty = await FacultyCredentials.findById(facultyId).select('name email employeeId');
    }
    ...
    if (req.user.role === 'faculty' && allocationObj.faculty && allocationObj.FacultyCredentials._id.toString() !== req.user._id.toString()) {
...
router.get('/ical/:allocationId', async (req, res) => {
  try {
    ...
    let faculty = null;
    if (facultyId) {
      faculty = await FacultyCredentials.findById(facultyId).select('name email');
    }

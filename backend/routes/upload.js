...
// @route   POST /api/upload/faculty-credentials/preview
// @desc    Preview faculty credentials (parse and validate without saving)
// @access  Private/Admin
router.post('/faculty-credentials/preview', protect, authorize('admin'), upload.single('file'), async (req, res) => {
  try {
    if (!req.file) {
      return res.status(400).json({
        success: false,
    ...
    const fileType = req.file.originalname.endsWith('.csv') ? 'csv' : 'excel';
    const result = await fileUploadService.previewFacultyCredentials(req.file.path, fileType);
...
// @route   POST /api/upload/faculty-credentials
// @desc    Upload faculty credentials (CSV/Excel) - creates login accounts with generated passwords
// @access  Private/Admin
router.post('/faculty-credentials', protect, authorize('admin'), upload.single('file'), async (req, res) => {
  try {
    if (!req.file) {
      return res.status(400).json({
        success: false,
    ...
    const fileType = req.file.originalname.endsWith('.csv') ? 'csv' : 'excel';
    const result = await fileUploadService.uploadFacultyCredentials(req.file.path, fileType);
    ...
    if (result.errors && result.errors.length > 0) {
      ...
      return res.status(200).json({
        success: true,
        message: `Created ${result.credentialsCreated} accounts with ${result.errors.length} errors`,
        data: result,
        warnings: result.errors
      });
    }

    // Show email results summary
    const emailSuccess = result.emailResults?.filter(e => e.success).length || 0;
    const emailFailed = result.emailResults?.filter(e => !e.success).length || 0;

    console.log(`✅ Created ${result.credentialsCreated} faculty accounts`);
    console.log(`📧 Emails sent: ${emailSuccess} successful, ${emailFailed} failed`);

    res.json({
      success: true,
      message: `Successfully created ${result.credentialsCreated} faculty accounts. Emails sent: ${emailSuccess}/${result.credentialsCreated}`,
      data: result
    });
  } catch (error) {
    // Clean up file on error

...
const { Admin, Hod, FacultyCredentials, findUserById } = require('../utils/userHelper');
...
/**
 * Helper function to populate faculty data from both collections
 * Checks FacultyCredentials first, then FacultyUpload as fallback
 */
async function populateFacultyMap(facultyIds) {
  const facultyMap = new Map();

  if (facultyIds.length === 0) return facultyMap;

  // Try FacultyCredentials first (for login-enabled faculty)
  const facultyCredentials = await FacultyCredentials.find({ _id: { $in: facultyIds } })
    .select('name email employeeId department campus phone')
    .lean();
  ...
  // Then check FacultyUpload for any remaining IDs (fallback)
  const remainingIds = facultyIds.filter(id => !facultyMap.has(id));
  if (remainingIds.length > 0) {
    const facultyUploads = await FacultyUpload.find({ _id: { $in: remainingIds } })
      .select('name email employeeId department campus phone')
      .lean();
...
router.get('/dashboard', async (req, res) => {
  try {
    ...
    const [
      ...
      FacultyUpload.countDocuments(facultyFilter), // Get from 'faculties' collection
    ...
    if (result && result.success) {
      try {
        ...
        if (io) {
          ...
          if (facultyIds.length > 0) {
            const facultyMembers = await FacultyUpload.find({ _id: { $in: facultyIds } }).select('name email');
            facultyMembers.forEach(f => {
              facultyMap.set(f._id.toString(), f);
            });
          }
...
router.post('/timetable/notify', async (req, res) => {
  try {
    ...
    let faculty = await FacultyUpload.find(facultyFilter);

    console.log(`📋 Found ${FacultyUpload.length} faculty member(s) from 'faculties' collection`);

    // Don't filter by isActive - use all faculty regardless of isActive status
    console.log(`✅ Using all ${FacultyUpload.length} faculty members (not filtering by isActive)`);

    if (FacultyUpload.length === 0) {
      // Debug: Check what users exist in all collections
      const adminCount = await Admin.countDocuments();
      const hodCount = await Hod.countDocuments();
      const facultyCount = await FacultyUpload.countDocuments();
    ...
    res.json({
      ...
      data: {
        totalFaculty: FacultyUpload.length,
        successCount,
        failCount
      }
    });
...
router.post('/allocations/:allocationId/add-faculty', async (req, res) => {
  try {
    ...
    const faculty = await FacultyUpload.findById(facultyId);
    if (!faculty) {
      return res.status(404).json({
        success: false,
    ...
    if (allocation.FacultyUpload.toString() === facultyId) {
      return res.status(400).json({
        success: false,
    ...
    const facultyData = {
      _id: FacultyUpload._id,
      name: FacultyUpload.name,
      email: FacultyUpload.email,
      employeeId: FacultyUpload.employeeId,
      department: FacultyUpload.department,
      campus: FacultyUpload.campus
    };
...
// @route   GET /api/admin/faculty-credentials
// @desc    Get all faculty credentials with filters
// @access  Private/Admin
router.get('/faculty-credentials', async (req, res) => {
  try {
    const { campus, department, search } = req.query;
    const filter = {};
    ...
    const facultyCredentials = await FacultyCredentials.find(filter)
    ...
    res.json({
      success: true,
      count: facultyCredentials.length,
...
// @route   POST /api/admin/faculty-credentials/reset-password
// @desc    Reset password for faculty (single or all)
// @access  Private/Admin
router.post('/faculty-credentials/reset-password', async (req, res) => {
  try {
    const { facultyId, resetAll, campus, department } = req.body;
    ...
    if (resetAll) {
      // Reset all faculty credentials based on filters
      const filter = { isActive: true };
      if (campus) filter.campus = campus;
      if (department) filter.department = department;

      const facultyList = await FacultyCredentials.find(filter);

      for (const faculty of facultyList) {
        try {
          const newPassword = generatePassword(12);
          faculty.password = newPassword; // Will be hashed by pre-save hook
          await faculty.save();

          // Send email
          try {
            const emailResult = await emailService.sendFacultyCredentials({
              name: faculty.name,
              email: faculty.email,
          ...
          errors.push(`Failed to reset password for ${faculty.email}: ${error.message}`);
        }
      }

      res.json({
        success: true,
        message: `Reset ${resetResults.length} faculty passwords`,
        data: {
          resetCount: resetResults.length,
          emailsSent: resetResults.filter(r => r.emailSent).length,
      ...
      const faculty = await FacultyCredentials.findById(facultyId);
      ...
      res.json({
        success: true,
        message: `Password reset for ${faculty.name}`,
        data: {
          facultyId: faculty._id,
          email: faculty.email,
          name: faculty.name,
      ...
      return res.status(400).json({
        success: false,
        message: 'Either facultyId or resetAll must be provided'
      });
    }
  } catch (error) {
    res.status(500).json({
...
// @route   POST /api/admin/faculty-credentials/resend
// @desc    Resend credentials email to faculty (without changing password)
// @access  Private/Admin
router.post('/faculty-credentials/resend', async (req, res) => {
  try {
    const { facultyId, resendAll, campus, department } = req.body;
    const emailService = require('../services/emailService');
    ...
    if (resendAll) {
      // Resend to all faculty based on filters
      const filter = { isActive: true };
      if (campus) filter.campus = campus;
      if (department) filter.department = department;
      ...
      for (const faculty of facultyList) {
        try {
          // Note: We can't resend the original password as it's hashed
          // This endpoint should inform users to use reset-password instead
          resendResults.push({
            facultyId: faculty._id,
            email: faculty.email,
            name: faculty.name,
            skipped: true,
            reason: 'Password is hashed - use reset-password to generate new credentials'
          });
        } catch (error) {
          errors.push(`Failed to process ${faculty.email}: ${error.message}`);
        }
      }

      res.json({
        success: false,
        message: 'Cannot resend credentials - passwords are hashed. Use reset-password instead.',
        data: {
          processedCount: resendResults.length,
          results: resendResults,
          errors: errors.length > 0 ? errors : undefined
        }
      });
    } else if (facultyId) {
      // Single faculty resend
      const faculty = await FacultyCredentials.findById(facultyId);
      ...
      res.json({
        success: false,
        message: 'Cannot resend credentials - password is hashed. Use reset-password to generate new credentials.',
        data: {
          facultyId: faculty._id,
          email: faculty.email,
          name: faculty.name
      ...
      return res.status(400).json({
        success: false,
        message: 'Either facultyId or resendAll must be provided'
      });
    }
  } catch (error) {
    res.status(500).json({

...
router.get('/:id', async (req, res) => {
  try {
    ...
    if (allocation.faculty) {
      const faculty = await FacultyUpload.findById(allocation.faculty)
    ...
    // Faculty can only see their own allocations
    if (req.user.role === 'faculty' && allocationObj.faculty && allocationObj.FacultyUpload._id.toString() !== req.user._id.toString()) {

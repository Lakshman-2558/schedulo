...
const createFacultyUsers = async () => {
  try {
    ...
    while (continueCreating) {
      ...
      const existingFaculty = await FacultyCredentials.findOne({ email: facultyEmail.toLowerCase().trim() });
      if (existingFaculty) {
        ...
        if (overwrite.toLowerCase() === 'yes' || overwrite.toLowerCase() === 'y') {
          await FacultyCredentials.deleteOne({ email: facultyEmail.toLowerCase().trim() });
